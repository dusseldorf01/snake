...
describe('createAsyncReducer', () => {
  ...
  const asyncReducer = createAsyncReducer(initialAsyncState, asyncActions);
  const store = configureStore({ reducer: combineReducers({ asyncReducer }) });

  it('should has right initial state', () => {
    expect(store.getState().asyncReducer).toEqual(initialAsyncState);
  });

  it('should set right state on request', () => {
    const expectedState = { ...initialAsyncState, loading: true, params: { data: '123' } };
